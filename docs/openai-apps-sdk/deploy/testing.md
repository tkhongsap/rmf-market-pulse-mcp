...
Testing validates that your connector behaves predictably before exposing it to users. The guidance emphasizes three core areas: tool correctness, component UX, and discovery precision.

## Unit Test Your Tool Handlers

Exercise each tool function directly with representative inputs. Verify schema validation, error handling, and edge cases such as empty results or missing IDs. Include automated tests for authentication flows if you issue tokens or require linking. Keep test fixtures close to your MCP code so they stay current as schemas evolve.
...
4. Click **List Tools** and **Call Tool** to inspect raw requests and responses
...
- Toggle it on in a new conversation and run through your golden prompt set (direct, indirect, negative)
- Record when the model selects the right tool, what arguments it passed, and whether confirmation prompts appear as expected
